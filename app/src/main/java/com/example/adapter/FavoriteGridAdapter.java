...
public class FavoriteGridAdapter extends ArrayAdapter<ItemDb> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (objLatestBean.getVideoType().equals("local")) {
            Picasso.get().load(objLatestBean.getImageUrl()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        }
        else if (objLatestBean.getVideoType().equals("server_url")) {
            Picasso.get().load(objLatestBean.getImageUrl()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        } else if (objLatestBean.getVideoType().equals("youtube")) {
            Picasso.get().load(Constant.YOUTUBE_IMAGE_FRONT + objLatestBean.getvideoId() + Constant.YOUTUBE_SMALL_IMAGE_BACK).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        } else if (objLatestBean.getVideoType().equals("dailymotion")) {
            Picasso.get().load(Constant.DAILYMOTION_IMAGE_PATH + objLatestBean.getvideoId()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        } else if (objLatestBean.getVideoType().equals("vimeo")) {
            Picasso.get().load("" + objLatestBean.getImageUrl().toString()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        }
