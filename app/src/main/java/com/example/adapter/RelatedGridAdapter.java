...
public class RelatedGridAdapter extends ArrayAdapter<ItemRelated> {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (objLatestBean.getRVideoType().equals("local")) {
            Picasso.get().load(objLatestBean.getRImageUrl()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        } else if (objLatestBean.getRVideoType().equals("server_url")) {
            Picasso.get().load(objLatestBean.getRImageUrl()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        } else if (objLatestBean.getRVideoType().equals("youtube")) {
            Picasso.get().load(Constant.YOUTUBE_IMAGE_FRONT + objLatestBean.getRVideoId() + Constant.YOUTUBE_SMALL_IMAGE_BACK).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        } else if (objLatestBean.getRVideoType().equals("dailymotion")) {
            Picasso.get().load(Constant.DAILYMOTION_IMAGE_PATH + objLatestBean.getRVideoId()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        } else if (objLatestBean.getRVideoType().equals("vimeo")) {
            Picasso.get().load("" + objLatestBean.getRImageUrl().toString()).placeholder(R.drawable.placeholder).into(holder.imgv_latetst);
        }
